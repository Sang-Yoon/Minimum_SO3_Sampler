...
def main():
    ...
    if pcd_reg.GLOBAL_REGISTRATION:
        ...
        if pcd_reg.RANSAC:
            result_ransac = pcd_reg.execute_global_registration(src_down, dst_down, src_down_fpfh, dst_down_fpfh, voxel_size)
            print("Global registration took %.3f sec.\n" % (time.time() - start_global_registration))
            print(result_ransac)
            pcd_reg.draw_registration_result(src_down, dst_down, result_ransac.transformation)
        elif pcd_reg.FGR:
            result_fast = pcd_reg.execute_fast_global_registration(src_down, dst_down, src_down_fpfh, dst_down_fpfh, voxel_size)
            print("Fast global registration took %.3f sec.\n" % (time.time() - start_global_registration))
            print(result_fast)
            pcd_reg.draw_registration_result(src_down, dst_down, result_fast.transformation)
    ...
    if pcd_reg.LOCAL_REGISTRATION:
        ...
        if pcd_reg.VANILLA_ICP:
            ...
            result_local_refinement = pcd_reg.execute_multi_scale_ICP_registration(
